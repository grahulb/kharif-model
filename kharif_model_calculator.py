...
class KharifModelCalculator:
	...
	def output_zonewise_budget_to_csv_agri(self, zonewise_budget_csv_filename, PET_sum_cropend, PET_sum,rain_sum):
		...
		writer.writerow(['PET Monsoon End'] + [PET_sum[self.crop_name] if (ag_or_non_ag == 'agricultural' or ag_or_non_ag == 'zone' ) else PET_sum[other_LU_crops[t]] for ID in self.zonewise_budgets	for ag_or_non_ag in ['agricultural']	for t in self.zonewise_budgets[ID][ag_or_non_ag] ])
	...
	def output_zonewise_budget_to_csv(self, zonewise_budget_csv_filename, PET_sum_cropend, PET_sum,rain_sum):
		...
		writer.writerow(['PET Monsoon End'] + [PET_sum[self.crop_name] if (ag_or_non_ag == 'agricultural' or ag_or_non_ag == 'zone' ) else PET_sum[other_LU_crops[t]] for ID in self.zonewise_budgets	for ag_or_non_ag in ['agricultural', 'non-agricultural', 'zone']	for t in self.zonewise_budgets[ID][ag_or_non_ag] ])
